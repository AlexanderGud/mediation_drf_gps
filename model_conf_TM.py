...
# with a hidden confounder affecting T and M.
...
def gen_M(e, X, T, H, Hpost=0):
    return (2 + H + np.cos(T) + X[:, 0] - X[:, 1] + 2 * X[:, 3] + e[:, 3]).reshape(
        (len(X), 1)
    )
...
modelname = "model_conf_TM_h" + str(h) + "_enorm_p" + str(p) + "_n" + str(n)
