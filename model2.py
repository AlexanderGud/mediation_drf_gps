...
def gen_T(e, X, H=0):
    return np.maximum(0, 3 + X[:, 0] + X[:, 1] + X[:, 2] + e[:, 1])
...
def gen_M(e, X, T, H=0, Hpost=0):
    return (3 + np.sqrt(T) + X[:, 0] - X[:, 1] + 2 * X[:, 3] + e[:, 3]).reshape(
        (len(X), 1)
    )


def gen_Y(e, X, T, M, H=0, Hpost=0):
    return M[:, 0] + np.tanh(T) - 0.5 * X[:, 0] - 2 * X[:, 2] - X[:, 3] + e[:, 4]
...
modelname = "model2_enorm_p" + str(p) + "_n" + str(n)
mediation_evaluation(model, n, p, modelname, n_jobs=n_jobs, seed=30)

modelname = "model2_covadj_enorm_p" + str(p) + "_n" + str(n)
