# The data generating model corresponding to Model 7 in the thesis.
...
def gen_M(e,X,T,H=0,Hpost=0):                                                                        
    ...
    return (2 + np.sqrt(np.maximum(0,T)) + (covar*coeff).sum(axis=1) + e[:,3]).reshape((len(X),1))   
...
def gen_Y(e,X,T,M,H=0,Hpost=0):                                                                      
    ...
    return M[:,0] * np.cos(T) + (covar*coeff).sum(axis=1) + e[:,4]                                   
