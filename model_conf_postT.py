...
# with a post-treatment hidden confounder affecting M and Y.
...
def gen_M(e, X, T, H=0, Hpost=0):
    return (2 + np.cos(Hpost) + X[:, 0] - X[:, 1] + 2 * X[:, 3] + e[:, 3]).reshape(
        (len(X), 1)
    )
...
modelname = "model_conf_postT_h" + str(h) + "_enorm_p" + str(p) + "_n" + str(n)
