...
def gen_M(e, X, T, H=0, Hpost=0):
    ...
    return (0.5 * T + (covar * coeff).sum(axis=1) + e[:, 3]).reshape((len(e), 1))
...
def gen_Y(e, X, T, M, H=0, Hpost=0):
    ...
    return 3 * M[:, 0] + 1 * T + (covar * coeff).sum(axis=1) + e[:, 4]
...
modelname = "model_benchmark_linear_enorm_p" + str(p) + "_n" + str(n)
...
modelname = "model_benchmark_linear_et_p" + str(p) + "_n" + str(n)
mediation_evaluation(model, n, p, modelname, n_jobs=n_jobs, seed=30)
