...
def gen_M(e, X, T, H=0, Hpost=0):
    ...
    coeff3 = 1 / np.sqrt(q) * np.linspace(-1, 1, 3 * q) * np.resize([1], 3 * q)
    covar = X[:, np.hstack([np.arange(2 * q), np.arange(3 * q, 4 * q)])]
    return np.array(
        [
            (T + (covar * coeff1).sum(axis=1) + e[:, 3]),
            (-np.sqrt(np.maximum(0, T)) + (covar * coeff3).sum(axis=1) + e[:, 4]),
            (np.arctan(T) + (covar * coeff2).sum(axis=1) + e[:, 5]),
        ]
    ).transpose()
...
modelname = "model5_enorm_p" + str(p) + "_n" + str(n)
