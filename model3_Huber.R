# Model 3 in the thesis run with the estimator proposed by Huber et al.
...
gen_M = function(e,X,Tr,H=0,Hpost=0){
  2 + cos(Tr) + X[,1] - X[,2] + 2*X[,4] + e[,4]
}
gen_Y = function(e,X,Tr,M,H=0,Hpost=0){
  3*sqrt(abs(M)) - atan(Tr) - 0.5*X[,1] - 2*X[,3] - X[,4] + e[,5]
}

gen_e = gen_error_norm
modelname = paste0('model3_Huber_enorm_p', p, '_n', n) 
mediation_evaluation_Huber()

gen_e = gen_error_t
modelname = paste0('model3_Huber_et_p', p, '_n', n) 
mediation_evaluation_Huber()
