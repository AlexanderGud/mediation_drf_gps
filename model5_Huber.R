# Model 5 in the thesis run with the estimator proposed by Huber et al.
...
gen_M = function(e,X,Tr,H=0,Hpost=0){
  ...
  coeff3 = 1/sqrt(q)*seq(-1,1,length.out=3*q)*rep_len(c(1),3*q)
  covar = X[,c(1:(2*q),(3*q+1):(4*q))]
  cbind((Tr + covar%*%coeff1 + e[,4]),
         (-sqrt(pmax(0,Tr)) + covar%*%coeff2 + e[,5]),
         (atan(Tr) + covar%*%coeff3 + e[,6])
         )
} 
gen_Y = function(e,X,Tr,M,H=0,Hpost=0){
  ...
  cos(M[,1] + M[,2]) - M[,3] - sqrt(pmax(0,Tr)) + covar%*%coeff + e[,7]
}

gen_e = gen_error_norm
modelname = paste0('model5_Huber_enorm_p', p, '_n', n) 
