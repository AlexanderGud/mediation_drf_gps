...
gen_M = function(e,X,Tr,H=0,Hpost=0){
  ...
  2 + cos(Tr) + covar%*%coeff + e[,4]
}
gen_Y = function(e,X,Tr,M,H=0,Hpost=0){
  q = ncol(X)/4
  coeff = 1/sqrt(q)*seq(2,1,length.out=3*q)*rep_len(c(1,1-1,-1),3*q)
  covar = X[,c(1:q,(2*q+1):(4*q))]
  3*sqrt(abs(M)) - atan(Tr) + covar%*%coeff + e[,5]
}

gen_e = gen_error_norm
modelname = paste0('model_benchmark_nonlinear_Huber_enorm_p', p, '_n', n) 
mediation_evaluation_Huber()

gen_e = gen_error_t
modelname = paste0('model_benchmark_nonlinear_Huber_et_p', p, '_n', n) 
mediation_evaluation_Huber()
